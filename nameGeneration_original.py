...
filename = 'Dataset/all.txt'
filenameTrain = 'Dataset/train.txt'
filenameTest = 'Dataset/test.txt'
...
def char_tensor(string):
    ...
    for c in range(len(string)):
        tensor[c] = n_letters.index(string[c])
...
def testing(decoder, nb_samples, lineTest, percent):
    ...
    if nb_samples > 0:
        ...
        for i in range(1, nb_samples + 1):
            ...
            progress(total=nb_samples, acc=accuracy,
                     start=start, epoch=i, l=len(lineTest))
        ...
        while accuracy < p:
            ...
            i = i + 1
            progressPercent(totalNames=l, start=start,
                            names=accuracy, p=percent, samplesGenerated=i)
...
if __name__ == '__main__':
    ...
    #
    parser.add_argument("-d", "--trainingData", default="Dataset/all.txt", type=str,
                        help="trainingData [path/to/the/data]")
    parser.add_argument("-te", "--trainEval", default='train',
                        type=str, help="trainEval [train, eval, test]")
    #
    parser.add_argument("-r", "--run", default="rnnGeneration",
                        type=str, help="name of the model saved file")
    ...
    parser.add_argument('--bidirectional', default=True,
                        type=bool, help="Bidirectionnal model [default True]")
    ...
    decoder = RNN(n_letters, args.hidden_size,
                  n_letters, args.num_layers).to(device)
    # RNNLight(n_letters, 128, n_letters).to(
    #     device)
    ...
    modelFile = args.run + "_" + \
        str(args.num_layers) + "_" + str(args.hidden_size) + "_" + \
        str(args.learning_rate).replace('.', '_') + \
        "_" + str(args.max_epochs) + ".pt"

    modelPath: str = f"{args.model.split('/')[0]}/{modelFile}"

    if not path.exists(modelPath.split('/')[0]):
        makedirs(basename(modelPath.split('/')[0]))

    # torch.serialization.add_safe_globals([decoder])
